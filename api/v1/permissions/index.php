	...
	try {
		if($_SERVER['REQUEST_METHOD'] === "GET") {
			if($path === "/acrapi/api/v1/permissions/") {
				$result = sendRequest($restAPIBaseURL.'/api.php/permissions','GET',headers: [
					"Authorization" => $headers["Authorization"] ?? ''
				]);
				echo $result;
			}
		} elseif($_SERVER['REQUEST_METHOD'] === "POST") {
			$result = sendRequest($restAPIBaseURL.'/api.php/permissions','POST', file_get_contents('php://input'),headers: [
					"Authorization" => $headers["Authorization"] ?? ''
				]);
			echo $result;
			...
			if (preg_match("#^/acrapi/api/v1/permissions/(\d+)/users/(\d+)/?$#",$normalized_path, $matches)) {
				$result = sendRequest($restAPIBaseURL."/api.php/permissions/$matches[1]/users/$matches[2]",'PUT', file_get_contents('php://input'),headers: [
					"Authorization" => $headers["Authorization"] ?? ''
				]);
			}
